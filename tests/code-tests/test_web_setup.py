...
def test_setup_succeeds_with_packaged_assets(mocker, tmp_path):
    ...
    assert web_setup.run_setup() is True


def test_setup_fails_when_packaged_assets_missing(mocker, tmp_path):
    assets_dir = tmp_path / "web_static"
    assets_dir.mkdir()
    ...
    assert web_setup.run_setup() is False
