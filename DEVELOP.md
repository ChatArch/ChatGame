...
- Keep the public root explicitly named `chatgame`; expose `--version`, `--tree`, and `--tree-brief`.
- Use ChatStyle `add_tree_option()` and the registered Click renderer instead of a package-local tree implementation.
- ChatGame currently has no env/profile/config behavior. If that changes, register a typed ChatEnv provider and use `chatenv>=0.2.10,<0.3.0` with ChatEnv-managed storage paths.
